...
- The tier gate has two sources: a Frappe Cloud Marketplace subscription (the plan is read from press and cached on the Investment License single) or, off Cloud, an offline Ed25519 key. Whichever grants more wins, and resolving a tier never makes a network call. Tiers are tracking limits — a number of asset classes, plus an optional portfolio-value cap in a reference currency — checked server-side when a Security is created (including broker-sync auto-creation). Existing data is never locked; over-limit usage surfaces as a dashboard banner. See [tiers.md](tiers.md).
